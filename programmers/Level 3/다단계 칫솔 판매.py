def solution(enroll, referral, seller, amount):
    ...
    for s, a in zip(seller, amount):
        pay = a*100
        dic[s][1] += int(pay*90/100)
        while True:
            pay = int(pay*10/100)
            if dic[s][0] == '-':
                break
            elif int(pay*10/100) <= 0:
                dic[dic[s][0]][1] += pay
                break
            else:
                dic[dic[s][0]][1] += pay - int(pay*10/100)
                s = dic[s][0]
    
    return [v[1] for v in dic.values()]


### 메모
# 주어진 대로 풀면 해결 된다.
# 1. 번 돈의 90퍼센트를 가진다.
# 2. 나머지 10퍼센트가 0이 아니면 나머지의 90퍼센트를 부모의 돈에 추가한다.
# 3. 나머지 10퍼센트가 0이 되거나 부모가 없을 때 까지 위를 반복한다.
# 4. 부모가 없으면 반복문을 종료한다.
# 5. 나머지 10퍼센트가 0이면 남은 금액 모두를 부모의 돈에 추가한다.
