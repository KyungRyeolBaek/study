...
# print(pyupbit.get_ohlcv(test, interval='minute1', count=2)['close'])
...
def save_earning_rate(df):
    earning_rate = df.sort_values(by = '수익', axis = 0, ascending=False)
    print(earning_rate)
    # 수익률 csv 파일로 저장
    earning_rate.to_csv('./earning_rate.csv')

구매한코인가격 = dict()
수익률 = pd.DataFrame(data = [[0, 0, 0, 0, 0, 0]]*len(tickers), columns=['시작금액', '현재금액', '구매가격', '수익', '총구매액', '수익률'], index=tickers)
수익률['시작금액'] = round(upbit.get_balance('KRW'), 2)

while True:
    print(구매한코인가격)
    save_earning_rate(수익률)
    수익률['현재금액'] = round(upbit.get_balance('KRW'), 2)
    while time.sleep(61 - datetime.datetime.now().second):
        for item in tickers:
            coin = upbit.get_balance(item)
            if coin:
                ...
                이전가 = pyupbit.get_ohlcv(item, interval='minute1', count=2)['close'][0]
                if 현재가 < 이전가:
                    if item in 구매한코인가격: 
                        if 현재가 > (구매한코인가격[item]*1.001):
                            upbit.sell_market_order(item, coin)
